...
export default function BookingForm({ preselectedRoom, variant = 'card' }: BookingFormProps) {
  ...
  useEffect(() => {
    async function fetchCalendar() {
      ...
      try {
        const res = await fetch(`/api/calendar?room=${roomSlug}`)
  ...
  function validate(): boolean {
    ...
    if (hasDateConflict) {
      setError(lang === 'it' ? 'Le date selezionate non sono disponibili' : 'Selected dates are not available')
      return false
    }
  ...
  async function handleStripeCheckout() {
    ...
    try {
      ...
      if (!checkoutRes.ok || !checkout.url) {
        // Stripe not configured — show error and fallback to WhatsApp
        if (checkout.error?.includes('not configured') || checkout.error?.includes('Stripe')) {
          setError(
            lang === 'it' 
              ? 'Stripe neconfigurat. Se redirectioneaza catre WhatsApp...' 
              : 'Stripe not configured. Redirecting to WhatsApp...'
          )
          setTimeout(() => handleWhatsApp(), 2000)
          return
        }
  ...
  // Format booked dates for the date input min/max
  ...
  const inputCls = 'w-full px-4 py-3.5 bg-ivory border border-border rounded-none text-navy text-base md:text-sm focus:outline-none focus:border-gold transition-colors touch-manipulation'

  return (
    <div className={isCard ? 'bg-white rounded-none p-8 md:p-10 ' : ''}>
      {isCard && (
        <h3 className="text-2xl md:text-3xl font-normal tracking-tighter text-navy mb-8 leading-tight">
      ...
      <div className={`grid gap-5 ${isCard ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-2 md:grid-cols-4'}`}>
        ...
        <div>
          ...
          <input 
            type="date" 
            value={checkOut} 
            min={getNextAvailableDate(checkIn || today)} 
        ...
        <div>
          ...
          <select 
            ...
            onChange={(e) => { 
              ...
              const r = property.rooms.find(r => r.slug === e.target.value); 
              if (r && guests > r.capacity.guests) setGuests(r.capacity.guests); 
              setError('') 
            }} 
        ...
        <div>
          ...
          <select 
            value={guests} 
            onChange={(e) => { setGuests(Number(e.target.value)); setError('') }} 
            className={`${inputCls} appearance-none`}
          >
      ...
      <div className="mt-4 flex items-center gap-3">
        {calendarLoading ? (
          <div className="flex items-center gap-2 text-sm text-[#9e9790]">
            <span className="w-3 h-3 border-2 border-[#9e9790]/30 border-t-[#9e9790] rounded-full animate-spin" />
            {lang === 'it' ? 'Caricamento disponibilità...' : 'Loading availability...'}
          </div>
        ) : bookedDates.length > 0 ? (
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-amber-500" />
            <span className="text-sm text-[#9e9790]">
              {lang === 'it' 
                ? `${bookedDates.length} notti prenotate per questa camera` 
                : `${bookedDates.length} nights booked for this room`}
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#9e9790]">
              {lang === 'it' ? 'Disponibilità aggiornata' : 'Availability updated'}
            </span>
          </div>
        )}
      </div>
      ...
      {nights > 0 && (
        <div className="mt-6 p-4 bg-ivory rounded-none flex items-center justify-between">
          <div className="text-sm text-[#9e9790]">
            {nights} {nights === 1 ? (lang === 'it' ? 'notte' : 'night') : (lang === 'it' ? 'notti' : 'nights')}
          </div>
          <div className="text-right">
            <p className="text-2xl font-normal text-navy tracking-tight">€{estimatedPrice}</p>
      ...
      {hasDateConflict && (
        <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-none">
          <p className="text-sm text-red-600">
            {lang === 'it' 
              ? '⚠️ Alcune date selezionate non sono disponibili. Scegli altre date.' 
              : '⚠️ Some selected dates are not available. Please choose different dates.'}
          </p>
        </div>
      )}

      {error && !hasDateConflict && <p className="text-sm text-red-600 mt-4">{error}</p>}
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-4 min-h-[56px] bg-navy text-ivory text-sm font-medium tracking-[0.1em] uppercase rounded-none hover:bg-navy/80 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 touch-manipulation"
        >
          {loading ? (
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-4 min-h-[56px] bg-[#25D366] text-white text-sm font-medium tracking-[0.1em] uppercase rounded-none hover:bg-[#20bd5a] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 touch-manipulation"
        >
