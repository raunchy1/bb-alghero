    ...
    <>
      ...
      <section className="pt-32 pb-16 bg-white border-b border-border">
        <div className="container-luxury">
          ...
          <h1 className="font-serif text-2xl sm:text-display-lg text-navy mb-4">{t({ it: 'Servizi', en: 'Amenities' })}</h1>
      ...
      <section className="bg-white py-16 border-y border-border">
        <div className="container-luxury">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              <p className="text-xs font-medium tracking-widest uppercase text-muted mb-4">{t({ it: 'Regole', en: 'Rules' })}</p>
              <h2 className="font-serif text-xl sm:text-display-sm text-navy mb-6">{t({ it: 'Regole della casa', en: 'House rules' })}</h2>
            </div>
            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div>
                <h3 className="text-sm font-medium text-navy mb-3">Check-in / Check-out</h3>
              ...
              <div>
                <h3 className="text-sm font-medium text-navy mb-3">{t({ it: 'Durante il soggiorno', en: 'During your stay' })}</h3>
                <ul className="space-y-2 text-sm text-muted">
              ...
              <div>
                <h3 className="text-sm font-medium text-navy mb-3">{t({ it: 'Incluso nel soggiorno', en: 'Included in your stay' })}</h3>
                <ul className="space-y-2 text-sm text-muted">
              ...
              <div>
                <h3 className="text-sm font-medium text-navy mb-3">{t({ it: 'Cancellazione', en: 'Cancellation' })}</h3>
                <ul className="space-y-2 text-sm text-muted">
      ...
      <section className="py-24">
        <div className="container-luxury">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div>
              <p className="text-xs font-medium tracking-widest uppercase text-muted mb-4">{t({ it: 'Prezzi', en: 'Prices' })}</p>
              <h2 className="font-serif text-xl sm:text-display-sm text-navy mb-6">{t({ it: 'Tariffe', en: 'Rates' })}</h2>
            ...
            <div className="space-y-4">
              {listingData.pricing.seasonalPricing.map((season) => (
                <div key={season.season} className="flex items-center justify-between py-4 px-6 bg-white border border-border">
                  <div>
                    <p className="font-medium text-navy text-sm">{season.season}</p>
                    <p className="text-xs text-muted mt-0.5">
                      {season.months.map(m => new Date(2024, m - 1).toLocaleString('it', { month: 'short' })).join(', ')}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-serif text-xl text-navy">€{season.price}</p>
                    <p className="text-xs text-muted">{t({ it: 'per notte', en: 'per night' })}</p>
                  </div>
                </div>
              ))}
              <div className="py-4 px-6 bg-sand border border-border">
                ...
                <div className="flex justify-between text-sm text-navy font-medium mt-3 pt-3 border-t border-border">
